...
const Map: FC = () => {
  ...
  const position: LatLngExpression = [
    locations[activeBuild]?.lat === undefined ? 53.9177318 : locations[activeBuild].lat,
  ...
  function LocationMarker() {
    ...
    return (
      <Marker position={position} icon={LocationIcon}>
        <Popup>
          {locations[activeBuild]?.name === undefined
            ? 'Выберите место'
            : locations[activeBuild].name}
          <br />
        ...
        <ul>
          {locations.map((location) => (
            <li
              ...
              onClick={() => {
                activeBuild === location.structureId
                  ? setActiveBuild(11)
                  : setActiveBuild(location.structureId);
              }}
